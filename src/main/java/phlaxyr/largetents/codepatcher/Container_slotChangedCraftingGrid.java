...
import static phlaxyr.largetents.codepatcher.PatchTransformer.get;
import static phlaxyr.largetents.codepatcher.PatchTransformer.obfdesc;
...
//		mv.visitMethodInsn(INVOKESTATIC, "phlaxyr/largetents/craftingarrow/ArrowManager", "injectArrowResult", "(Lnet/minecraft/inventory/Container;Lnet/minecraft/inventory/InventoryCrafting;Lnet/minecraft/item/ItemStack;Lnet/minecraft/world/World;)Lnet/minecraft/item/ItemStack;", false);
//		mv.visitVarInsn(ASTORE, 6);
		...
		insns.add(new MethodInsnNode(INVOKESTATIC, 
				"phlaxyr/largetents/craftingarrow/ArrowManager", 
				"injectArrowResult", 
	...
	public void onCodeNotFound() {
		LargeTents.lumberjack.error("NOT FOUND! LargeTents can't seem to find the code line \"res.setInventorySlotContents(0, itemstack);\"!");
		throw new IllegalStateException("LargeTents can't seem to find the code line \"res.setInventorySlotContents(0, itemstack);\"!");
		
	}
	public void inject(MethodNode method, boolean obf) {
		LargeTents.lumberjack.info("Attempting to patch "+method.name);
		...
		LargeTents.lumberjack.info("Patching "+method.name+" Complete!");
	}
