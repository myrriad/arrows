...
//    public abstract int gridY();
     ...
     * 
     * @param clump
     * @param gridSlotLengthX
     * @param gridSlotLengthY
     */
    public ContainerCrafterXbyX(InventoryPlayer inv, T tile, World world, BlockPos pos, 
    		CraftXbyXManager<R> manager, int gridSlotLengthX, int gridSlotLengthY, IPlayerSlotTracker ptracker, ICraftingPosTracker ctracker) {
    	...
    	*/
        this.craftResult = new InventoryTileCraftResult(this.getTile());
    	this.craftMatrix = new InventoryTileCrafting(this, gridSlotLengthX, gridSlotLengthY, this.getTile());

    	this.resultX = ctracker.resultX();
    	this.resultY = ctracker.resultY();
    	this.gridX = ctracker.gridX();
    	this.gridY = ctracker.gridY();
    	//crafting output
        ...
        int index = 0;
        //crafting grid
		for (int i = 0; i < gridSlotLengthY; ++i) { //# of rows
			for (int j = 0; j < gridSlotLengthX; ++j) { //# of slots in each row
				this.addSlotToContainer(new Slot(this.craftMatrix,    index++/*j + i * gridX*/,
						
						gridX + j * 18, gridY + i * 18));
				LargeTents.lumberjack.debug("Added a slot");;
			}
		}
